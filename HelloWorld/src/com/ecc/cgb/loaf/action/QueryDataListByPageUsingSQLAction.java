...
/**
 * <b>功能描述：</b><br>
 * 根据输入的preparedStatement分页查询多笔数据，将结果存入到iColl中<br>
 * <b>参数说明:</b><br>
 * queryFields:出现在查询结果中的列<br>
 * querySql: 查询用的SQL<br>
 * sortParam:排序条件<br>
 * inputFields:输入参数列表，顺序必须和SQL中?的顺序一致<br>
 * outputFields:输出参数列表<br>
 * iCollName:保存查询结果的Icoll的名称<br>
 * beginPosFieldName:存放起始条数字段名称，默认为beginPos<br>
 * ShowNumFieldName:存放起始条数字段名称，默认为showNum<br>
 * totalNumFieldName:存放起始条数字段名称，默认为totalNum<br>
 * 
 * @version 1.1 2008-03-16
 * @author zhanghao@yuchengtech.com
 */
public class QueryDataListByPageUsingSQLAction extends LianaJDBCAction {
	/**
	 * 字段分隔符
	 */
	...
	{	
		...
		try
		{
			...
			String finalSQL = LoafTools.parseQuerySql(context, this.querySql, input);
